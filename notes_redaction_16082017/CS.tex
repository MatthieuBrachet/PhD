...
On pose $\alpha$ l'angle géodésique entre $\mathbf{x}_{i,j}^{(K)}$ et $\mathbf{x}_{0,j}^{(K)}$. $\beta$ est l'angle géodésique entre $\mathbf{x}_{i,j}^{(K)}$ et $\mathbf{x}_{i,0}^{(K)}$ (Voir figure \ref{fig: alpha beta}).
...
On se concentre ici sur le panel I, alors $K=I$ et $\mathbf{x}_{i,j}^{(K)}=\mathbf{x}_{i,j}^{(I)}$ est un point du panel I de coordonnées $(x,y,z)$ dans $\mathbb{R}^3$. Alors les relations suivantes sont vérifiées :
...
\begin{proof}
Conséquence des relations entre $(x,y,z)$ et des relations $(\xi, \beta)$ et $(\alpha, \eta)$ par panel (tableau \ref{tab: x y z fct de xi eta alfa beta}).
\end{proof}
