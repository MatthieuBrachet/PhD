...
On pose $\alpha$ l'angle géodésique entre $\mathbf{x}_{i,j}^{(K)}$ et $\mathbf{x}_{0,j}^{(K)}$. $\beta$ est l'angle géodésique entre $\mathbf{x}_{i,j}^{(K)}$ et $\mathbf{x}_{i,0}^{(K)}$ (Voir figure \ref{fig: alpha beta}). Ainsi, les relations suivantes sont vérifiées :
...
Si $K=I$, $\mathbf{x}_{i,j}^{(K)}=\mathbf{x}_{i,j}^{(I)}$ est un point du panel I de coordonnées $(x,y,z)$ dans $\mathbb{R}^3$. Alors les relations suivantes sont vérifiées :
