...
\begin{tikzpicture}[scale=2]
	...
	\draw  (0,-1.4-8) node[left] {$y$} ;
	\draw [>=stealth, ->] (0,0-8) -- (1.4,0-8) ;
	\draw  (1.4,0-8) node[right] {$x$} ;
	...
	\draw  (4,1.4-8) node[right] {$y$} ;
	\draw [>=stealth, ->] (4,0-8) -- (4+1.4,0-8) ;
	\draw  (4+1.4,0-8) node[right] {$x$} ;
...
\begin{tikzpicture}[scale=2.2]
    \draw[>=stealth, ->,color=green, domain=-79:-68] plot({1.05*cos(\x)},{0.2*sin(\x)});
    \draw[color=green] (.45,-.3) node{$\xi$} ;
    \draw[>=stealth, ->,color=red, domain=-7:35] plot({0.1*cos(\x)},{0.95*sin(\x)});
...
\end{tikzpicture}
\end{center}
\caption{Le panel I est constitué des points d'intersection d'un ensemble de grands cercles. Sur un panel, un point $\mathbf{M}$ est localisé par $\xi$ et $\eta$.}
\label{fig: panel I}
\end{figure}  
