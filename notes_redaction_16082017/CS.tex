...
Le grand cercle $C_i^{(1)}$ correspond à une iso-ligne $\xi$ constante et le grand cercle $C_j^{(2)}$ à une iso-ligne $\eta$ constante. Considérons l'iso-ligne $\eta$ constante sur le panel I. Sur ce panel, l'angle $\alpha$ est tel que :

\begin{equation}
- \alpha_0 (\eta) \leq \alpha \leq \alpha_0(\eta) \text{ avec } \alpha_0( \eta) = \arctan \left( \dfrac{\sqrt{2}}{2} \tan \eta \right).
\end{equation}

Cependant le grand cercle $C_j^{(2)}$ passe sur les autres panels II, III et IV dans cet ordre. On peut donc prolonger $\alpha$ comme étant l'angle curviligne le long du grand cercle complet. Sur le panel II, le grand cercle $C_j^{(2)}$ coupe d'autres grands cercles (ceux permettant de construire le panel II) en $\mathbf{M}_k$ de coordonnées $(\xi, \eta) = ()$.
...
De plus, on peut calculer les vecteurs $\mathbf{g}_{\alpha}$, $\mathbf{g}_{\beta}$, $\mathbf{g}^{\alpha}$ et $\mathbf{g}^{\beta}$. Par commodité, on écrira ces vecteurs en fonction de $\mathbf{g}_{\xi}$, $\mathbf{g}_{\eta}$, $\mathbf{g}^{\xi}$ et $\mathbf{g}^{\eta}$ :
...
\end{itemize}
\label{eq: g_alpha g_beta fct de g_xi g_eta}
\end{proposition}
...
\begin{proof}
Les égalités suivantes est vérifiées grâce à \eqref{eq: g_alpha g_beta fct de g_xi g_eta} et \eqref{eq: derivee partiel link} :
