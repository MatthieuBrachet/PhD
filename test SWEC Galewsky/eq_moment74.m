...
for dim=1:3
    q_fI=squeeze(vt_fI(:,:,dim));
    q_fII=squeeze(vt_fII(:,:,dim));
    q_fIII=squeeze(vt_fIII(:,:,dim));
    q_fIV=squeeze(vt_fIV(:,:,dim));
    q_fV=squeeze(vt_fV(:,:,dim));
    q_fVI=squeeze(vt_fVI(:,:,dim));
    [grl_fI,grl_fII,grl_fIII,grl_fIV,grl_fV,grl_fVI]=...
        gr74(q_fI,q_fII,q_fIII,q_fIV,q_fV,q_fVI,n,nn);
    [lap_fI(:,:,dim),lap_fII(:,:,dim),lap_fIII(:,:,dim),lap_fIV(:,:,dim),lap_fV(:,:,dim),lap_fVI(:,:,dim)]=...
...
for kk=1:3
    ...
    eq1_fI(:,:,kk)=-grad_I(:,:,kk)-(vort_fI+f_I).*vect_fI(:,:,kk)+visc*lap_fI(:,:,kk);
    
    f_II=2.*omega.*(-cos(lambda_fII).*cos(teta_fII).*sin(alpha)+sin(teta_fII).*cos(alpha));
    eq1_fII(:,:,kk)=-grad_II(:,:,kk)-(vort_fII+f_II).*vect_fII(:,:,kk)+visc*lap_fII(:,:,kk);
    
    f_III=2.*omega.*(-cos(lambda_fIII).*cos(teta_fIII).*sin(alpha)+sin(teta_fIII).*cos(alpha));
    eq1_fIII(:,:,kk)=-grad_III(:,:,kk)-(vort_fIII+f_III).*vect_fIII(:,:,kk)+visc*lap_fIII(:,:,kk);
    
    f_IV=2.*omega.*(-cos(lambda_fIV).*cos(teta_fIV).*sin(alpha)+sin(teta_fIV).*cos(alpha));
    eq1_fIV(:,:,kk)=-grad_IV(:,:,kk)-(vort_fIV+f_IV).*vect_fIV(:,:,kk)+visc*lap_fIV(:,:,kk);
    
    f_V=2.*omega.*(-cos(lambda_fV).*cos(teta_fV).*sin(alpha)+sin(teta_fV).*cos(alpha));
    eq1_fV(:,:,kk)=-grad_V(:,:,kk)-(vort_fV+f_V).*vect_fV(:,:,kk)+visc*lap_fV(:,:,kk);
    
    f_VI=2.*omega.*(-cos(lambda_fVI).*cos(teta_fVI).*sin(alpha)+sin(teta_fVI).*cos(alpha));
    eq1_fVI(:,:,kk)=-grad_VI(:,:,kk)-(vort_fVI+f_VI).*vect_fVI(:,:,kk)+visc*lap_fVI(:,:,kk);
