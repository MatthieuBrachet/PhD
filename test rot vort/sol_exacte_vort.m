function [vt,vortvt] = sol_exacte_vort(x,y,z)
...
for i=1:n2
    for j=1:n2
        er=[x(i,j) y(i,j) z(i,j)]/r;
        rot(1:3)=cross(glambda(i,j,1:3),dulambda(i,j,1:3))+cross(gteta(i,j,1:3),duteta(i,j,1:3));
        vortvt(i,j)=dot(rot,er);
    end
end
