function [u,v,h,Ps,T]=paldor_init(lat,lon,lev,time,waveFlag)
...
%     waveFlag  - one of the strings: 'WIG', 'Rossby' or 'EIG' 
...
%      
    ...
    function [psi,dpsi]=getPsi(lat)
        ...
        psi  = amp.*cp.^sigma.*G5;
        dpsi = amp.*cp.^(sigma).*(-sigma.*tp.*G5 + G5p);  
    ...
    function [u,v,h]=getAmplitudes(lat,waveFlag)
        ...
        switch waveFlag
            case 'Rossby'
                ...
                h  = sqrt(2.*omega.*a.^2.*H0.^2.*abs(Km))./Km.*(dpsi+tp.*(0.5.*Kp./Km-2.*omega./C).*psi);
                u  = 2.*omega.*sp./C.*V + g./(a.*C.*cp).*h;
            case {'EIG','WIG'}
                W1 = ((kw.*C).^2-(2.*omega.*sp).^2)./(C.*cp); 
                W2 = ((kw.*C).^2-(2.*omega.*sp).^2)./(2.*omega.*cp.^2); 
                V  = sqrt(2.*omega.*g.*H0.*abs(W1))./(W1.*cp).*(dpsi+tp.*(0.5-2.*omega./W2+2.*omega./C).*psi);
    ...
    function [u,v,h]=getFields(lat,lon,lev,time,waveFlag)
        ...
        for t=1:nt
            for l=1:nl
                u(:,:,nl,t) = real(uTilde.*exp(1i.*kw.*(lon-C.*time(t))));
                v(:,:,nl,t) = real(vTilde.*exp(1i.*kw.*(lon-C.*time(t))));
                h(:,:,nl,t) = real(hTilde.*exp(1i.*kw.*(lon-C.*time(t))));    
            end
        end
