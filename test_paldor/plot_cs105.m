function []=plot_cs105(n,nn,funfIe,funfIIe,funfIIIe,funfIVe,funfVe,funfVIe)
% classic plot en projection stéréographique couleur en centrant le 0 sur
% la figure
...
surf(lam_VI_qI,the_VI_qI,funfVIsI(nn:-1:1,:));hold on;axis([lmin lmax temin temax umin umax]);
...
xline=1.28;
plot3(lam_I(1,:),the_I(1,:),funfIe(1,:)+eps,'k','LineWidth',xline); hold on;
plot3(lam_I(nn,:),the_I(nn,:),funfIe(nn,:)+eps,'k','LineWidth',xline); hold on;
plot3(lam_I(:,1),the_I(:,1),funfIe(:,1)+eps,'k','LineWidth',xline); hold on;
plot3(lam_I(:,nn),the_I(:,nn),funfIe(:,nn)+eps,'k','LineWidth',xline); hold on;

plot3(lam_II(1,:),the_II(1,:),funfIIe(1,:)+eps,'k','LineWidth',xline); hold on;
plot3(lam_II(nn,:),the_II(nn,:),funfIIe(nn,:)+eps,'k','LineWidth',xline); hold on;
plot3(lam_II(:,1),the_II(:,1),funfIIe(:,1)+eps,'k','LineWidth',xline); hold on;
plot3(lam_II(:,nn),the_II(:,nn),funfIIe(:,nn)+eps,'k','LineWidth',xline); hold on;

lam_III=lam_III+2*pi*(lam_III<-eps);
plot3(lam_III(1,:),the_III(1,:),funfIIIe(1,:)+eps,'k','LineWidth',xline); hold on;
plot3(lam_III(nn,:),the_III(nn,:),funfIIIe(nn,:)+eps,'k','LineWidth',xline); hold on;
plot3(lam_III(1:(nn+1)/2,1),the_III(1:(nn+1)/2,1),funfIIIe(1:(nn+1)/2,1)+eps,'k','LineWidth',xline); hold on;
plot3(lam_III((nn+1)/2:nn,1)-2*pi,the_III((nn+1)/2:nn,1),funfIIIe((nn+1)/2:nn,1)+eps,'k','LineWidth',xline); hold on;
plot3(lam_III(1:(nn+1)/2,nn),the_III(1:(nn+1)/2,nn),funfIIIe(1:(nn+1)/2,nn)+eps,'k','LineWidth',xline); hold on;
plot3(lam_III((nn+1)/2:nn,nn)-2*pi,the_III((nn+1)/2:nn,nn),funfIIIe((nn+1)/2:nn,nn)+eps,'k','LineWidth',xline); hold on;

plot3(lam_IV(1,:),the_IV(1,:),funfIVe(1,:)+eps,'k','LineWidth',xline); hold on;
...
ZCercle=umax*ones(size(XCercle));
