...
radcoast=(radius);
[xc,yc,zc]=sph2cart(long*pi/180+2*pi,lat*pi/180,radcoast*ones(sz));


% PANEL I
xc_fI=xc((xc>=(sqrt(2)/2*radcoast)));
yc_fI=yc((xc>=(sqrt(2)/2*radcoast)));
zc_fI=zc((xc>=(sqrt(2)/2*radcoast)));

XC=yc_fI./xc_fI;
YC=zc_fI./xc_fI;
I=ones(size(XC));

plot3(XC,YC,radcoast*I,'k-','MarkerSize',.2);

% PANEL II
xc_fII=xc((yc>=(sqrt(2)/2*radcoast)));
yc_fII=yc((yc>=(sqrt(2)/2*radcoast)));
zc_fII=zc((yc>=(sqrt(2)/2*radcoast)));

XC=-xc_fII./yc_fII;
YC=zc_fII./yc_fII;
I=ones(size(XC));

plot3(XC+2,YC,radcoast*I,'k.','MarkerSize',.2);

% PANEL III
xc_fIII=xc(xc<-sqrt(2)/2*radcoast);
yc_fIII=yc(xc<-sqrt(2)/2*radcoast);
zc_fIII=zc(xc<-sqrt(2)/2*radcoast);

XC=yc_fIII./xc_fIII;
YC=-zc_fIII./xc_fIII;
I=ones(size(XC));

plot3(XC+4,YC,radcoast*I,'k.','MarkerSize',.2);

% PANEL IV
xc_fIV=xc(yc<-sqrt(2)/2*radcoast);
yc_fIV=yc(yc<-sqrt(2)/2*radcoast);
zc_fIV=zc(yc<-sqrt(2)/2*radcoast);
