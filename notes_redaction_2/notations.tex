...
Les notations employées sont celles de la partie \ref{sec:notation_1D} dans le contexte périodique avec $a=0$ et $b=L$.
...
L'opérateur $\tau_p$ agit sur les fonctions périodiques $u$ par :
\begin{equation}
\tau_p u(x_j) = (\tau_p u^*)_j = u^*_{j+p} = u(x_{j+p}).
\end{equation}
en considérant les propriétés de périodicité lorsque $j+p > N$ ou $j+p<1$.
...
\begin{proposition}
les valeurs propres de $\tau$ sont $\omega^k$ avec
\begin{equation}
...
\begin{equation}
(\tau \mathfrak{u}^k)_j = \exp \left[ i (j+1) \dfrac{2 \pi k}{N} \right] = \exp \left[ i j \dfrac{2 \pi k}{N} \right] \exp \left[ i \dfrac{2 \pi k}{N} \right]  = \omega^k \mathfrak{u}_j^k
\end{equation}
d'où le résultat.
\end{proof}
...
Chaque fonction propre est associé à la valeur propre $P(\omega^k)$ avec 
...
\begin{equation}
(P(\tau) \mathfrak{u}^k)_j = \left( \gsum_{n=0} a_i \tau_n \mathfrak{u}^k \right)_j = \gsum_{n=0} a_n (\lambda^k)^n \mathfrak{u}^k_j = P(\omega^k) \mathfrak{u}^k_j
\end{equation}
d'où le résultat.
\end{proof}
...
avec 
\begin{equation}
\vec_1 ( \mathfrak{u} ) =\gsum_{j=1}^N \mathfrak{u}_j \mathbf{e}_j.
\end{equation}
\end{definition}
...
\begin{corollaire}
\begin{itemize}
\item Les valeurs propres de $T$ sont les valeurs $(\lambda^k)_{-N/2+1 \leq k \leq N/2}$. 
Chaque valeur propre est associée à un vecteur propre $U^k$ vérifiant :
\begin{equation}
U^k = \vec_1 (\mathfrak{u}^k )
\label{eq:eigenvectorT}
\end{equation}
où $\mathfrak{u}^k$ et $\omega$ sont données dans la proposition \ref{prop:eigen_tau}.
...
\begin{equation}
L_h u^* - R_h (Lu)^* = \mathcal{O} \left ( h^{\alpha} \right)
\end{equation}
avec $R_h$ un opérateur d'interpolation et $L_h$ un opérateur d'approximation de $L$.
\end{definition}

\subsubsection{Opérateur centré d'ordre 2}
...
\vdots\\
\mathfrak{u}_{N}\\
\end{bmatrix} \in \mathbb{R}^N \text{ et } U_x = \vec_1( \delta_x \mathfrak{u}) = \begin{bmatrix}
