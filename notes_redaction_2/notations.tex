...
En utilisant les notations de la partie \ref{sec:notation_2D} en contexte périodique, nous élargissons la définition des opérateurs 1D en dimension 2
...
\left\lbrace
\begin{array}{rcl}
\tau_x u_{i,j} & = & u_{i+1,j}\\
\tau_y u_{i,j} & = & u_{i,j+1}\\
\end{array}
\right.
\end{equation}
avec $\mathbf{u}$ une fonction de grille et $1 \leq i,j \leq N$.
\end{definition}
