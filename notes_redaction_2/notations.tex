...
\begin{equation}
\mu^k = \dfrac{1}{2h} \left[ \exp \left( \dfrac{2 i \pi k}{N} \right) - \exp \left( \dfrac{-2 i \pi k}{N} \right) \right] = \dfrac{i}{h} \sin \left( \dfrac{2 \pi k}{N} \right) \text{ avec } 0 \leq k \leq N-1.
\end{equation}
En particulier, on remarque que $\mu^N = 0$ donc la matrice $K$ n'est pas inversible.
...
Un tel opérateur vérifie :

\begin{proposition}
Soit $u : x \in \Omega \mapsto u(x) \in \mathbb{R}$ une fonction de $\mathcal{C}^5( \Omega)$ alors, en notant $u^*$ la fonction de grille associée et $u'^*$ la fonction de grille associée à sa dérivée première, on a 
\begin{equation}
\sigma_x u'^*_j - \delta_x u^*_j = \dfrac{h^4}{180} u^{(5)}(\xi_j)
\end{equation}
\end{proposition}

De plus, l'opérateur $\sigma_x$ est inversible, en effet...


On en déduit que ... THM DE CONSISTANCE
