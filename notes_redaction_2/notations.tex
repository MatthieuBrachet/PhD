...
\delta_x \mathfrak{u}_0\\
\delta_x \mathfrak{u}_1\\
\vdots\\
\delta_x \mathfrak{u}_{N-1}\\
...
\begin{equation}
\theta^k = \dfrac{1}{2h} \left[ \exp \left( \dfrac{2 i \pi k}{N} \right) - \exp \left( \dfrac{-2 i \pi k}{N} \right) \right] = \dfrac{i}{h} \sin \left( \dfrac{2 \pi k}{N} \right) \text{ avec } 0 \leq k \leq N-1.
\end{equation}
...
\end{equation}
avec $0 \leq k \leq N-1$. Chaque valeur propre est associé à la fonction propre $\mathfrak{u}^k$ avec 
\begin{equation}
\mathfrak{u}_j^k = \exp \left[ i j \dfrac{2 \pi k}{N} \right]
\end{equation}
\label{prop:vp_sigma}
...
\begin{equation}
\mu^k \neq 0 \text{ pour tout } 0 \leq k \leq N-1
\end{equation}
...
\begin{equation}
\sigma_x u'^* - \delta_x u^* = \dfrac{h^4}{144} \left( u^{(5)}(\xi'_i) + u^{(5)}(\eta'_i) \right) - \dfrac{h^4}{240} \left( u^{(5)}(\xi_i) + u^{(5)}(\eta_i) \right)
\end{equation}
...
\begin{equation}
| u'^*_i -  \delta_x^H u^*_i |_{\infty} \leq C h^4
\end{equation}
\end{theoreme}
...
\begin{equation*}
\begin{array}{rcl}
| u'^*_i -  \delta_x^H u^*_i |_{\infty} & = & | \sigma_x^{-1} \left( \sigma_x u'^*_i - \delta_x u^*_i \right) |_{\infty} \\
	& \leq & \| \sigma^{-1}_x \|_{\infty} |\sigma_x u'^*_i - \delta_x u^*_i |_{\infty}\\
...
\subsection{Opérateur de translation en géométrie cartésienne}
...
\subsection{Opérateur centré d'ordre 2 en dimension 2}
...
\subsection{Opérateur hermitien en dimension 2}