...
Pour construire un test pour évaluer les performances du gradient approché $\nabla_{T, \Delta} \cdot$, certaines propriétés son utiles.
\begin{lemme}
...
\hline 
\hline
\textbf{ordre estimé}& $3.9705$ & $3.9652$ & $3.8609$ & $3.9822$ \\
\hline
\end{tabular}
\end{center}
...
La divergence utilisée dans \ref{def:divergence_disc} est couteuse en calcul. Il faut calculer 6 dérivées hermitiennes. Une méthode basée sur une divergence approchée moins coûteuse en calculs est donnée dans \cite{Croisille2015}. Cette méthode est basée sur le résultat suivant :
...
\begin{equation}
\mathbf{g}^{\xi} (\mathbf{x} ) = \dfrac{1}{1 + \dfrac{y^2}{x^2}} \begin{bmatrix}
- \dfrac{y}{x^2} \\
\dfrac{1}{x} \\
0
...
\psi_1(x) \\
0
\end{bmatrix} \text{ ainsi que } \sqrt{\tilde{\mathbf{G}}} = a^2 \dfrac{(1+\tilde{X}^2)(1+ \tilde{Y}^2)}{(1+\tilde{X}^2+\tilde{Y}^2)^{3/2}}
\label{eq:expression2_div_v2}
\end{equation}
avec $\tilde{X}=y \psi_1(x)$ et $Y = z \psi_2(x)$.
