function [ht_fI, ht_fII, ht_fIII, ht_fIV, ht_fV, ht_fVI,...
    vt_fI, vt_fII, vt_fIII, vt_fIV, vt_fV, vt_fVI] = iter_swe( ntest, ddtime , Tmax)
...
nn=n+2;
mod101
itermax=min([100000,floor(Tmax/ddtime)]);
...
while t<Tmax && iter<itermax
    ...
    clc; 
    disp(real([test iter min(itermax,floor(Tmax/ddt))]));
end
