...
\end{equation}
où $\alpha \in [ 0, 2 \pi[$ désigne l'angle $\widehat{\mathbf{x}_0  \mathbf{x}}$ dans le sens choisi (Fig. \ref{fig: grand cercle}). L'angle $\alpha$ est \textit{l'angle géodésique} entre $\mathbf{x}_0$ et $\mathbf{x}$.
...
Si l'on considère par exemple le panel $(I)$, un point $\mathbf{x}'=(x',y',z')$ de la face centré sur $F'$ est projeté en $\mathbf{x} = (x,y,z)$ un point du panel $(I)$ (Voir Fig. \ref{fig: projection gnomonique}). Chaque panel est la projection de l'une des face du cube.
