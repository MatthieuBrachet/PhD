...
global opt_ftr opt_ftr_div test
...
while t<Tmax & iter<itermax
    ...
    max(max([forh_fI, forh_fII, forh_fIII, forh_fIV, forh_fV, forh_fVI]))
    err_div(iter)=max(max([div_fI, div_fII, div_fIII, div_fIV, div_fV, div_fVI]));
    err_div(end)
    ...
    [~,~,~,~,~,~,nrmge]=...
      nrm72(h_fI, h_fII, h_fIII, h_fIV, h_fV, h_fVI, n,nn,str);
    err1(iter)=nrmger/nrmge;
    ...
    if strcmp(video,'yes')==1
        figure(100)
        plot_cs11(n,nn,htnew_fI-h_fI,htnew_fII-h_fII,htnew_fIII-h_fIII,htnew_fIV-h_fIV,htnew_fV-h_fV,htnew_fVI-h_fVI)
        title(['error at ', num2str(time(end)), 'days'])
        hold off;
...
legend('norm 1','norm 2','infty norm')
grid on
if sauvegarde==1
...
figure(5)
plot_cs11(n,nn,h_fI-ht_fI,h_fII-ht_fII,h_fIII-ht_fIII,h_fIV-ht_fIV,h_fV-ht_fV,h_fVI-ht_fVI);
