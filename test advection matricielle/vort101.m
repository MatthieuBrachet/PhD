...
%% --- ASSEMBLAGE ---------------------------------------------------------
% FACE I: CALCUL DU GRADIENT EN FONCTION DES DERIVEES ALPHA ET BETA
rot_I(1:nn,1:nn,1)=dgxi_fI(:,:,3).*gxi_I(:,:,2) + dgeta_fI(:,:,2).*geta_I(:,:,3);
rot_I(1:nn,1:nn,2)=dgxi_fI(:,:,3).*gxi_I(:,:,1) + dgeta_fI(:,:,1).*geta_I(:,:,3);
rot_I(1:nn,1:nn,3)=dgxi_fI(:,:,2).*gxi_I(:,:,1) + dgeta_fI(:,:,1).*geta_I(:,:,2);

vort_I(1:nn,1:nn)=rot_I(1:nn,1:nn,1).*gr_I(1:nn,1:nn,1)+rot_I(1:nn,1:nn,2).*gr_I(1:nn,1:nn,2)+rot_I(1:nn,1:nn,3).*gr_I(1:nn,1:nn,3);

% FACE II: CALCUL DU GRADIENT EN FONCTION DES DERIVEES ALPHA ET BETA
rot_II(1:nn,1:nn,1)=dgxi_fII(:,:,3).*gxi_II(:,:,2) + dgeta_fII(:,:,2).*geta_II(:,:,3);
rot_II(1:nn,1:nn,2)=dgxi_fII(:,:,3).*gxi_II(:,:,1) + dgeta_fII(:,:,1).*geta_II(:,:,3);
rot_II(1:nn,1:nn,3)=dgxi_fII(:,:,2).*gxi_II(:,:,1) + dgeta_fII(:,:,1).*geta_II(:,:,2);

vort_II(1:nn,1:nn)=rot_II(1:nn,1:nn,1).*gr_II(1:nn,1:nn,1)+rot_II(1:nn,1:nn,2).*gr_II(1:nn,1:nn,2)+rot_II(1:nn,1:nn,3).*gr_II(1:nn,1:nn,3);

% FACE III: CALCUL DU GRADIENT EN FONCTION DES DERIVEES ALPHA ET BETA
rot_III(1:nn,1:nn,1)=dgxi_fIII(:,:,3).*gxi_III(:,:,2) + dgeta_fIII(:,:,2).*geta_III(:,:,3);
rot_III(1:nn,1:nn,2)=dgxi_fIII(:,:,3).*gxi_III(:,:,1) + dgeta_fIII(:,:,1).*geta_III(:,:,3);
rot_III(1:nn,1:nn,3)=dgxi_fIII(:,:,2).*gxi_III(:,:,1) + dgeta_fIII(:,:,1).*geta_III(:,:,2);

vort_III(1:nn,1:nn)=rot_III(1:nn,1:nn,1).*gr_III(1:nn,1:nn,1)+rot_III(1:nn,1:nn,2).*gr_III(1:nn,1:nn,2)+rot_III(1:nn,1:nn,3).*gr_III(1:nn,1:nn,3);

% FACE IV: CALCUL DU GRADIENT EN FONCTION DES DERIVEES ALPHA ET BETA
rot_IV(1:nn,1:nn,1)=dgxi_fIV(:,:,3).*gxi_IV(:,:,2) + dgeta_fIV(:,:,2).*geta_IV(:,:,3);
rot_IV(1:nn,1:nn,2)=dgxi_fIV(:,:,3).*gxi_IV(:,:,1) + dgeta_fIV(:,:,1).*geta_IV(:,:,3);
rot_IV(1:nn,1:nn,3)=dgxi_fIV(:,:,2).*gxi_IV(:,:,1) + dgeta_fIV(:,:,1).*geta_IV(:,:,2);

vort_IV(1:nn,1:nn)=rot_IV(1:nn,1:nn,1).*gr_IV(1:nn,1:nn,1)+rot_IV(1:nn,1:nn,2).*gr_IV(1:nn,1:nn,2)+rot_IV(1:nn,1:nn,3).*gr_IV(1:nn,1:nn,3);

% FACE V: CALCUL DU GRADIENT EN FONCTION DES DERIVEES ALPHA ET BETA
rot_V(1:nn,1:nn,1)=dgxi_fV(:,:,3).*gxi_V(:,:,2) + dgeta_fV(:,:,2).*geta_V(:,:,3);
rot_V(1:nn,1:nn,2)=dgxi_fV(:,:,3).*gxi_V(:,:,1) + dgeta_fV(:,:,1).*geta_V(:,:,3);
rot_V(1:nn,1:nn,3)=dgxi_fV(:,:,2).*gxi_V(:,:,1) + dgeta_fV(:,:,1).*geta_V(:,:,2);

vort_V(1:nn,1:nn)=rot_V(1:nn,1:nn,1).*gr_V(1:nn,1:nn,1)+rot_V(1:nn,1:nn,2).*gr_V(1:nn,1:nn,2)+rot_V(1:nn,1:nn,3).*gr_V(1:nn,1:nn,3);

% FACE VI: CALCUL DU GRADIENT EN FONCTION DES DERIVEES ALPHA ET BETA
rot_VI(1:nn,1:nn,1)=dgxi_fVI(:,:,3).*gxi_VI(:,:,2) + dgeta_fVI(:,:,2).*geta_VI(:,:,3);
rot_VI(1:nn,1:nn,2)=dgxi_fVI(:,:,3).*gxi_VI(:,:,1) + dgeta_fVI(:,:,1).*geta_VI(:,:,3);
rot_VI(1:nn,1:nn,3)=dgxi_fVI(:,:,2).*gxi_VI(:,:,1) + dgeta_fVI(:,:,1).*geta_VI(:,:,2);

vort_VI(1:nn,1:nn)=rot_VI(1:nn,1:nn,1).*gr_VI(1:nn,1:nn,1)+rot_VI(1:nn,1:nn,2).*gr_VI(1:nn,1:nn,2)+rot_VI(1:nn,1:nn,3).*gr_VI(1:nn,1:nn,3);

[vort_I,vort_II,vort_III,vort_IV,vort_V,vort_VI]=...
    ds101(vort_I,vort_II,vort_III,vort_IV,vort_V,vort_VI,n,nn);
