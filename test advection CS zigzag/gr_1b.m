...
ptsp=[0;0];
for i=1:2:nn-1
    ptsp=[ptsp(1,:) beta(i,1:nn) beta(i+1,nn:-1:1) ; ptsp(2,:) alfa(1,:,i) beta(nn:-1:1,i+1)];
    funspline=[funspline funfII(i,1:nn) funfII(i+1,nn:-1:1)];
end
betaspline(1)=ptsp(1,1);
for i=2:length(ptsp);
    if mod(nn,i) == 0 
        betaspline(i)=betaspline(i-1)+abs(ptsp(2,i)-ptsp(2,i-1));
    else
        betaspline(i)=betaspline(i-1)+abs(ptsp(1,i)-ptsp(1,i-1));
    end
end
pts=[];
for i=1:2:nn-1
...
ppspline=spline(betaspline, funspline);
fun=ppval(ppspline, betacrspl);
for i=1:2:nn-1
    va_fI(nn-1+i,1:nn)=fun((i-1)*(nn+1)+2:i*(nn+1));
    va_fI(nn+i,1:nn)=fun((i+1)*(nn+1):-1:i*(nn+1)+2);
end
