...
\end{equation}
$\rho(A)$ est le \textit{rayon spectral} de $A$.
...
Nous distinguons $l^2_h$, l'espace des fonctions de grilles d'une part, et d'autre part l'espace vectoriel $\mathbb{R}^{N+1}$ même si ces deux espaces sont isomorphes.
...
Pour tout $\mathfrak{v} \in l^2_{h,\per}$, on note $(\hat{\mathfrak{v}}_k )_{-N/2 \leq k \leq N/2}$ les composantes de $\mathfrak{v}$ sur la base $(\mathfrak{u}^k)_{-N/2 +1 \leq k \leq N/2}$ :
\begin{equation}
...
\end{equation}
où $p_k$ est le projecteur orthogonal sur $\Vect (\mathfrak{u}^k)$. Pour tout $\mathfrak{v} \in l^2_{h,\per}$ et $-N/2+1 \leq k \leq N/2$, on a
\begin{equation}
...
D'autre part, au voisinage $]x_{j-1}, x_{j+1}[$ de $x_j$ et en posant $t=x-x_j$, on a 
\begin{equation}
...
\end{proof}
Dans la suite, nous aurons besoin d'inverser l'opérateur $\sigma_x$. La propriété suivante permet de s'assurer de son inversibilité.
\begin{corollaire}
...
\begin{eqsys}
\sigma_{x} = \dfrac{9}{15} \Id + \dfrac{1}{5}\left( \tau_1 + \tau_{-1} \right) \\
\delta_{x} =\dfrac{14}{15} \dfrac{\tau_1 - \tau_{-1}}{2h} + \dfrac{1}{15} \dfrac{\tau_2 - \tau_{-2}}{4h}
\label{eq:comp6}
\end{eqsys}
...
\textbf{Exemples : }
\begin{itemize}
\item $Q_4^H(X)$ est donné par
\begin{equation}
Q_4^H(X) = \dfrac{X-X^{N-1}}{\dfrac{4}{3} + \dfrac{1}{3}(X+X^{N-1})},
\label{eq:polhermi4}
\end{equation}
\item $Q_6^H(X)$ est donné par
\begin{equation}
Q_6^H(X) = \dfrac{\dfrac{14}{15}(X-X^{N-1}) + \dfrac{1}{15}(X^2-X^{N-2})}{\dfrac{9}{15} + \dfrac{1}{5}(X+X^{N-1})},
\end{equation}
\item $Q_8^H(X)$ est donné par
\begin{equation}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[height=6.5cm]{compact_freq.jpg}
\end{center}
...
La matrice $P_{\sigma}$ est inversible si $|\beta |<1/2$ et trivialement symétrique. 
...
D'autre part, il joue le rôle d'une dissipation numérique. Cette contrainte se traduit par le fait que $\mathfrak{u}^{N/2}$, qui vérifie
\begin{equation}
\mathfrak{u}^{N/2}_j = (-1)^j \text{ pour } 0 \leq j \leq N-1,
\end{equation}
est inclus dans le noyau de $\mathcal{F}_{2J,x}$ : $\mathcal{F}_{2J,x}(\mathfrak{u}^{N/2}) = \mathfrak{0}$. On note que l'on a alors
\begin{equation}
...
\begin{proposition}
Si les $J$ coefficients $(a_p)_{1 \leq p \leq J}$ vérifient \eqref{eq:ftr_consistance}, \eqref{eq:ftr_filtrage} et \eqref{eq:ftr_precision}, c'est à dire
\begin{equation}
\left\lbrace
\begin{array}{rcl}
\gsum_{p=0}^J a_p & = & 1 \\
...
La Table \ref{tab:filter} résume les valeurs de $a_p$ associées à $\mathcal{F}_{2J,x}$ pour un ordre de précision $2J$ fixé.
\begin{table}[htbp]
\begin{center}
...
Les valeurs propres et fonctions propres de $\mathcal{F}_{x,2J}$ sont issues de la proposition \ref{prop:eigen_Ptau} appliquée au polynôme $S_{2J} \in \mathbb{R}_J [X]$ défini par \eqref{eq:pol_filtrage}.
...
\begin{itemize}
\item il existe un unique polynôme $P \in \mathbb{R}_{J}(X)$ tel que
\begin{equation}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[height=7.5cm]{freq_filter.jpg}
\end{center}
...
Les points $(x_i,y_j)_{0 \leq i,j \leq N}$ sont de deux types (voir Fig. \ref{fig:maillage2D}) :
\begin{itemize}
...
\begin{enumerate}
\item Une \textit{fonction de grille} est une fonction définie aux points de la grille $(x_i,y_j)_{0 \leq i,j \leq N}$. Nous notons ces fonctions en fonte gothique comme $\mathfrak{u}$ ou $\mathfrak{v}$. On note :
\begin{equation}
...
\item Soit $u : (x,y) \in \Omega \mapsto u(x,y) \in \mathbb{R}$, nous définissons la fonction de grille associée, notée $u^*$, comme la restriction de $u$ à la grille :
\begin{equation}
...
On considère dans cette section les fonctions de grille périodiques $u \in L^2_{h,\per}$. Nous définissons les opérateurs agissant sur les fonctions de grilles comme suit.
...
Les opérateurs obtenus en dimension 1 sont définis en dimension 2 grâce à ces deux opérateurs de translation. Les opérateurs centrés permettant d'approcher les dérivées partielles d'ordre 1 sont donnés par 
\begin{eqsys}
...
L'opérateur hermitien en dimension 1 : $\delta_x^H$ est étendu en dimension 2 grâce à la relation suivante 
\begin{equation}
\left\lbrace
\begin{array}{rcl}
...
Si $u : (x,y) \in \Omega \mapsto u(x,y) \in \mathbb{R}$ est une fonction de $\mathcal{C}^{2J}$ alors :
\begin{equation}
\left\lbrace
\begin{array}{rcl}
...
\textcolor{white}{
\begin{flushright}
\textit{"La mathématique est une science dangereuse : elle dévoile les supercheries et les erreurs de calcul."}, Galilée.
\end{flushright}
}