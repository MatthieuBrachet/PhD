...
elseif test == 5
    ...
    [lambda, teta, ~]=cart2sph(x,y,z);
    % u_lambda
%     uu = -sigma.*sqrt(gp.*hp)./10.*fun10(teta,-pi/4,pi/4).*exp(-sigma*t);
    uu = -sigma.*sqrt(gp.*hp)./10.*fun10(teta,teta0,teta1).*exp(-sigma*t);

    % u_teta
%     vv = 2*omega.*sqrt(gp*hp)/10.*sin(teta).*fun10(teta,-pi/4,pi/4).*exp(-sigma*t)+gp/radius.*dfun10(teta,-pi/4,pi/4).*exp(-sigma*t);
    vv = 2*omega.*sqrt(gp*hp)/10.*sin(teta).*fun10(teta,teta0,teta1).*exp(-sigma*t)+gp/radius.*dfun10(teta,teta0,teta1).*exp(-sigma*t);
