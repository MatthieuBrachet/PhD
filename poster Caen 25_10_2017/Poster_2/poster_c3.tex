...
\begin{multicols}{3} %%%DETERMINE LE NOMBRE DE COLONNES ****************************************************************
...
\begin{center}
\includegraphics[scale=0.8]{CS2.png}
...
Without the filtering procedure, the scheme is numerically fourth order accuracy and stable if 
\begin{equation}
\lambda = \dfrac{c \Delta t}{\Delta x} \leq \lambda_{\infty} = 2 \sqrt{\dfrac{2}{3}} \approx 1.6329
\end{equation}
\end{Theo}

With a filtering procedure of order $p$ in the form :
\begin{equation}
\mathcal{F}(h)_i = \gsum_{m=0}^M \dfrac{a_m}{2} \left( h_{i+m} + h_{i-m} \right)
\end{equation}

The scheme is numerically stable if $\lambda \leq \lambda_{p}$ with
$\lambda_{p}$ estimated numerically in Table 1.
...
\begin{center}
\includegraphics[scale=1]{ref_7369437806_snapshot_intermediaire599.png}\\
