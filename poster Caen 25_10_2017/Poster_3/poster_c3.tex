...
\begin{figure}
\begin{center}
\includegraphics[scale=.7]{CS2.png}
\caption{La Cubed-Sphere.}
\end{center}
\end{figure}

La discrétisation se fait par calcul de \textbf{dérivées hermitiennes} le long des grands cercles. L'utilisation de grands cercles permets de travailler en contexte périodique et d'éviter les conditions de bords.
...
\begin{itemize}
\item Caclul de \textbf{dérivée hermitienne} :
...
Si $f \in \mathcal{C}^5$,
\begin{equation}
f_{x,i} - f'(x_i) = \mathcal{O}(\Delta x^4)
\end{equation}
\end{Theo}
...
\begin{equation}
\mathcal{F}(f)_i = \gsum_{m=0}^{10} a_m \left( f_{i+m} + f_{i-m} \right)
\end{equation}
...
\item Discrétisation en temps : méthode de \textbf{Runge-Kutta d'ordre 4} filtré. On pose $q = [\mathbf{u},h]^T$.

\begin{enumerate}
\item $K^{(1)} = F_{\nabla} \left( q^n, t^n \right)$,
\item $K^{(2)} = F_{\nabla} \left( q^n + \dfrac{\Delta t}{2} K^{(1)}, t^n + \dfrac{\Delta t}{t}\right)$,
\item $K^{(3)} = F_{\nabla} \left( q^n + \dfrac{\Delta t}{2} K^{(2)}, t^n + \dfrac{\Delta t}{t}\right)$,
\item $K^{(4)} = F_{\nabla} \left( V^n + \Delta t K^{(3)}, t^n + \Delta t\right)$,  
...
Pour tout $1 \leq i \leq N$ et $n=0,1,...$, $h_i^n$ est une approximation de $h(t^n, x_i)$ obtenue avec le précédent schéma sur l'equation \eqref{eq:advection1D}. Par périodicité, on a $h_0^n = h_N^n$.
...
Sans filtre, le schéma est stable si 
\begin{equation}
...
\begin{equation}
\forall n = 0,1,... \hspace{1cm} \Delta x \gsum_{i} h_i^{n+1} =  \Delta x \gsum_{i} h_i^{n}.
\end{equation}
\end{Theo}
...
Le test de J. Galewsky \cite{G2004} est une perturbation d'une solution stationnaire. Ce test est difficile sur ce maillage
\begin{itemize}
...
\begin{center}
\includegraphics[scale=.8]{ref_7369437806_snapshot_intermediaire599.png}\\
\includegraphics[scale=.7]{ref_7369437806_massenergy.png}
\includegraphics[scale=.65]{ref_7369437806_enstrophy.png}

\textbf{Figure 3  : } Vorticité au jour 6, erreurs relatives de conservations sur une grille $6 \times 128 \times 128$.
\end{center}

\item \textbf{Test de Rossby-Haurwitz.} Le test de Rossby-Haurwitz est un test classique de climatologie numérique. La condition initiale se déplace d'Est en Ouest. Cette propriété est-elle vrai en temps long avec notre schéma?

\begin{center}
\includegraphics[scale=.8]{ref_7369145763_snapshot_intermediaire1399.png}\\
