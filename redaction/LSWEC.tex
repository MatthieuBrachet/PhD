...
l'équation est résolue sur la sphère $\mathbb{S}_a^2$ dont le rayon est $a$. $\mathbf{x}$, un point de $\mathbb{S}_a^2$ est un point de latitude $\lambda$ et de longitude $\theta$ the longitude. Un vecteur $\mathbf{v} : \mathbb{S}_a^2 \rightarrow \mathbb{T}\mathbb{S}_R^2$ est déterminé par :
...
Dans cette partie, nous considérons l'équation \eqref{LSWEC} sans forcage (\textit{i.e.} $\mathbf{F} \equiv \mathbf{0}$.
...
$$g \dfrac{\partial}{\partial t}  \| \eta \|^2_{L^2(\mathbb{S}_a^2)} + H \dfrac{\partial}{\partial t} \| \mathbf{u} \|^2_{L^2(\mathbb{S}_a^2)} = -2gH \gint_{\mathbb{S}_a^2} \left( \mathbf{u} \cdot \nabla \eta + \eta \nabla \cdot \mathbf{u} \right)$$

en utilisant \eqref{energy_eq3} et \ref{remark_stokes}, nous avons le résultat :

\begin{equation}
\dfrac{\partial}{\partial t} \left( g  \eta^2 + H \| u \|_{L^2(\mathbb{S}_a^2)}^2 \right) = 0
\end{equation}
\end{itemize}
\end{proof}
...
\dfrac{I^n}{I(0)}
\end{equation}
où  $I^n = \gint_{\mathbb{S}_a^2} \eta (t^n)$ la masse ou l'énergie $I^n = g \gint_{\mathbb{S}_a^2} \eta (t^n)^2 + H |u(t^n) |^2$ et $I(0)$ est la masse ou l'énergie initiale. Cette quantité $I^n/I(0)$ doit etre proche de $1$ si les quantités sont conservées.
\end{itemize} 
