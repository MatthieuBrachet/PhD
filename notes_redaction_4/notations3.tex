...
\right.
\end{equation}
de plus, les coefficients sont de signes alternées, c'est à dire $a_p a_{p+1}<0$.
\end{proposition}
...
\begin{array}{rcl}
\gsum_{p=1}^J a_p & = & 1 \\
\gsum_{p=1}^J a_p \dfrac{p^{2n}}{2n+1} & = & 2 \beta  \text{ pour } n=1,2,...P
\end{array}
\right.
...
\begin{multline}
(\delta_{x} u^*)_j - (\sigma_{x} (\partial_x u)^*)_j = \\  h^{2J+2} \left[ \dfrac{J}{(2J+3)!}  \left( \gsum_{p=1}^J a_p  p^{2J+2} \right)\partial_x^{(2J+3)}u(\theta)  - \left( \dfrac{2\beta}{(2J+2)!}   \right) \partial_x^{(2J+3)}u(\rho) \right]
\label{eq:eq_cons}
\end{multline} 
...
\begin{equation}
P_{\sigma} \vec (\delta_{2J+2,x}^H \mathfrak{u}) = D_{2J} \vec (\mathfrak{u})
\end{equation}
et on a 
\begin{equation}
...
Par calcul immédiat, on a :
\begin{equation}
(P^{-1}_{\sigma}D_{2J})^T = D_{2H}^T P_{\sigma}^{-T} = - D_{2J} P_{\sigma}^{-1} = - P_{\sigma}^{-1} D_{2J}.
\end{equation}
...
Il découle l'algorithme le calcul de $\vec (\delta_{2J+2,x}^H \mathfrak{u}) = P^{-1}_{\sigma} D_{2J} \vec (\mathfrak{u})$ donné par
