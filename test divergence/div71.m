function [div_fI,div_fII,div_fIII,div_fIV,div_fV,div_fVI]=...
    div71(mfunfI,mfunfII,mfunfIII,mfunfIV,mfunfV,mfunfVI,n,nn)
...
% FACE III: 
for jline1=1:nn,
    va_fI(2*nn-1:3*nn-3,jline1)=funfIII(1:nn-1,nn+1-jline1); 
end
% FACE IV:
for i=1:nn-1, 
   betaspline(1:nn)=beta(i,1:nn);
...
%figure(100); plot(vad_fI(1:nn,33)+vbd_fI(1:nn,33),'bo-')
...
for i=1:nn,
    for j=1:nn,
        xxtVa_VI(i,j)=-y_fVI(i,j)/z_fVI(i,j);                               %% -y_fVI(i,j)/z_fVI(i,j); 
...
for i=1:nn,
    for j=1:nn,
        xxtVb_VI(i,j)=-y_fVI(i,j)/z_fVI(i,j);                               % -y_fVI(i,j)/z_fVI(i,j);
...
[div_fI(1:nn,1:nn),div_fII(1:nn,1:nn),div_fIII(1:nn,1:nn),div_fIV(1:nn,1:nn),div_fV(1:nn,1:nn),div_fVI(1:nn,1:nn,1)]=...
    ds67(uwk_I,uwk_II,uwk_III,uwk_IV,uwk_V,uwk_VI,n,nn);