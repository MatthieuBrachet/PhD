...
Conditions \eqref{eq:consistency_condition_filter} and \eqref{eq:filtering_condition_filter} define a filter. We don't want to disrup so much the initial values, then we add the following accuracy conditions :
\beq
0 = \sum_{j=0}^J \dfrac{a_j}{2} (ij)^k\left( 1 + (-1)^k \right) \text{ for } k=1...K_{acc}.
\label{eq:accuracy_condition_filter}
\eeq

where $K_{acc} \in \mathbb{N}^{\star}$. When $K_{acc}$ is great the accuracy of the filter is great. Notice that for $k$ odd, the condition \eqref{eq:accuracy_condition_filter} become automatically true. We give some particular filters in the table \ref{tab:filter}.
...
$4$ & $10/16$ & $4/16$ & $-1/16$ & & & \\
\hline
$6$ & $44/64$ & $15/64$ & $-6/64$ & $1/64$ & & \\
\hline
$8$ & $186/256$ & $56/256$ & $-28/256$ & $8/256$ & $-1/256$ & \\
\hline
$10$ & $772/1024$ & $210/1024$ & $-120/1024$ & $45/1024$ & $-10/1024$ & $1/1024$ \\
\hline
\end{tabular}
\end{center}
...
\begin{proof}
First, notice the following inequality :
\beq
\max_{\theta \in [0,\pi)}
\left(
\vert \beta(e^{i \theta}) \vert \vert r(-\lambda m(e^{i\theta})) \vert
...
\max_{\theta \in [0,\pi)}
\left(
\vert \beta(e^{i \theta}) \vert \right) \max_{\theta \in [0,\pi)}
\left(\vert r(-\lambda m(e^{i\theta})) \vert
\right)
\eeq
In more, the following result :
\beq
\vert\beta ( e^{i \theta)}\vert = \vert \sum_{j=0}^J a_j \cos (j \theta) \vert \leq \vert \sum_{j=0}^J a_j \vert = 1
\eeq
...
\lambda \leq 2 \sqrt{\dfrac{2}{3}}
\eeq
using the proposition \ref{prop:cfl_sans_filtre}.
...
we use value $I(t)$ to measure conservation error because this quantity must be zero.
%% M. BRACHET M. BRACHET M. BRACHET M. BRACHET M. BRACHET M. BRACHET M. BRACHET M. BRACHET M. BRACHET
