...
$\mathbf{c} : (t,\mathbf{x}) \in (0, +\infty) \times \mathbb{S}^2_a \mapsto \mathbf{c}(t,\mathbf{x}) \in \mathbb{T}\mathbb{S}^2_a$ 
...
\right.
\end{equation}
where $\zeta = \left( \nabla_T \times \mathbf{v} \right) \cdot \mathbf{n} = \vort(\mathbf{v})$ 
...
\nabla_T \times \bv = 
\bg ^\xi \times \partial_\xi \bv  + \bg ^\eta \times \partial_\eta \bv 
\end{array}
\right.
\eeq
...
(\bg ^\xi)^k_{i,j}  \times (\bv)_{i,j}^k 
+
(\bg ^\eta)^k_{i,j} \times (\bv)_{i,j}^k  
\end{array}
\right.
\label{eq:div_rot}
\eeq
...
%% **********************************************************************************************************************************************
\subsection{Accuracy of the approximate spherical vorticity}

In practice, in the Shallow Water equation \eqref{eq:swe}, operators used are divergence, gradient and vorticity. The vorticity is the normal component of the rotationnal :
\begin{equation}
\zeta = \vort (\mathbf{v}) = \left( \nabla_T \times \mathbf{v} \right) \cdot \mathbf{n}
\end{equation}
with $\mathbf{v} : \mathbf{x} \in \mathbb{S}^2_a \mapsto \mathbf{v}(\mathbf{x}) \in \mathbb{T}\mathbb{S}_a^2$ and $\mathbf{n} = \mathbf{x}/a$ the normal exterior of the sphere $\mathbb{S}_a^2$.
The approximated vorticity is
\begin{equation}
\zeta_{i,j}^{(k)} = \vort_{h} (\mathbf{v}_{i,j}^{(k)}) = \left( \nabla_{T,\Delta} \times \mathbf{v}_{i,j}^{(k)} \right) \cdot \mathbf{n}(\mathbf{x}_{i,j}^{(k)})
\end{equation}

Using proposition \ref{prop:accuracy_op}, we know that each approximated operator is at least order 3.
Approximated gradient and divergence are introduced and studied in \cite{Croisille-10} and \cite{Croisille-7}.
The accuracy of the approximated vorticity is evaluate using the calculated vorticity of several tangential fields.

\subsubsection{Test 1 : }

We consider the tangential fields $\mathbf{x} \in \mathbb{S}_a^2 \mapsto \mathbf{v}(\mathbf{x}) \in \mathbb{T}\mathbb{S}_a^2$ with
...
The vorticity of this field is given by

\begin{equation}
\vort (\mathbf{v})(\mathbf{x}) = \dfrac{\alpha+1}{a} \cos^{\alpha-1} (\theta) \sin (\theta).
\end{equation}
It's important to take $\alpha \geq 2$ to avoid the pole singularity. We choose $\alpha = 3$ and observe the error

\begin{equation}
e_p = \dfrac{\| \vort_{\Delta}(\mathbf{v}_{i,j}^{(k)}) - \vort(\mathbf{v})_{i,j}^{(k)}  \|_p}{\| \vort(\mathbf{v})_{i,j}^{(k)}  \|_p}
\end{equation}
with $l \in \left\lbrace 1, 2, \infty \right\rbrace$.
Results obtained on this test case are given in tabular \ref{tab:rate1} and in figure \ref{fig:rate1}. The numerical results show fourth-order accuracy between the calculated and the exact vorticity.
...
Let $\mathbf{v} : \mathbf{x} \in \mathbb{R}^3 \mapsto \mathbf{v}(\mathbf{x}) \in \mathbb{R}^3$ is a regular vector field in $\mathbb{R}^3$ and $\hat{\mathbf{v}} = \mathbf{v}_{|\mathbb{S}_a^2}$ the restriction of $\mathbf{v}$ on the sphere $\mathbb{S}_a^2$. 
The following formula
\begin{equation}
\vort(\mathbf{v}) = \vort_{\mathbb{R}^3}(\mathbf{v})
\end{equation}

Then, for this test case, we take the vector field
\begin{equation}
\mathbf{v}(\mathbf{x}) = \mathbf{n}(\mathbf{x}) \times \begin{bmatrix}
\exp (y/a) \\ \exp (x/a) \\ \exp (z/a)
\end{bmatrix}.
\label{eq:test2}
\end{equation}
for each $\mathbf{x}\in \mathbb{R}^3$ and the vorticity is given by
\begin{equation}
...
For this test, the relative error is given in the table \ref{tab:rate2}. As for the previous test, the numerical results highlight a fourth-order accuracy.
...
\vort \left( \nabla_T h \right) = 0.
\end{equation}
The error is measured for $h(x,y,z)=x^3 y^2 z^3$ with different mesh parameters $N$. The numerical results are given in table \ref{tab:rate3} and are fourth-order accuracy.
...
where
\begin{equation}
\hat{u}_0^k=\frac{1}{L} \gint_0^{L} u_0(x)\exp(-i \tilde{k} x) dx
\end{equation}
