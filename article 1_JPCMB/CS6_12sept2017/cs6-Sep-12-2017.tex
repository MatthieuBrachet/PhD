...
\beq
\lambda \leq \dfrac{K_{RK4}}{\sqrt{3}}
\eeq
\end{prop}

\begin{proof}
The proof is based on \eqref{eq:33.09} and the some inequalities concerning the map $F: \theta \mapsto F(\theta) = \beta(e^{i \theta})$. Let $\theta \in [0;\pi)$ and $\alpha_f \in [0; 0.5)$. With the inequality \eqref{eq:consistency_condition_filter}, we show that :
\beq
F(\theta) \leq \dfrac{1}{1+2\alpha_f \cos \theta} \sum_{j=0}^J a_j = \dfrac{1+2 \alpha_f}{1+2\alpha_f \cos \theta}.
\eeq
...
That's to say, the scheme with filtering is, at least, as good as the scheme without filtering. Different filter are used in this study, it's summarized in the tabular REF.
...
\right\rbrace.
\eeq
The results are given in the tabular \ref{tab:CFL_explicit_filter} for the explicit filter (\textit{i.e. } $\alpha_f = 0$) and in tabular \ref{tab:CFL_implicit_filter} for implicit filter order 10 in function of $\alpha_f$.
\begin{table}
\begin{tabular}{|c|c|}
\hline
\textbf{Filter order} & \textbf{CFL} \\
\hline \hline
$\infty$ & $1.6329$ \\
$10$ & $1.6883$ \\
$8$ & $1.7114$ \\
...
\hline
\end{tabular}
\caption{CFL condition for the scheme \eqref{eq:SF.55.19} in function of the explicit filter order. $\infty$ denotes the no filter scheme. The CFL is obtained numerically.}
\label{tab:CFL_explicit_filter}
\end{table}
...
\hline
\end{tabular}
\caption{CFL condtion for the scheme \eqref{eq:SF.55.19} in function of $\alpha_f$. The CFL is obtained numerically.}
\label{tab:CFL_implicit_filter}
\end{table}

In the tabular \ref{tab:CFL_explicit_filter}, we see that the CFL condition seems to converge to the CFL condition without filter. It's a consequence of the better representation of hight frequencies of the hight order filter. A filter with a low order permit to take a greater $\Delta t$ than an hight order filter.
For an implicit filter ($\alpha_f \in ]0; 0.5[$), the tabular \ref{tab:CFL_implicit_filter} permit to see that the CFL number converge to the CFL number for the no filtering scheme when $\alpha_f$ converge to $0.5$. If $\alpha_f = 0$, the filter is explicit. 
