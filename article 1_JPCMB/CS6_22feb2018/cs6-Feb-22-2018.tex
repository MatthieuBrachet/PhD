...
$\vort_{T,\Delta}(\bvv)$ with $\bvv$ in \eqref{eq:test1} with the Hermitian vorticity. The error used the relative error is norm \eqref{eq:40.04} with $p=1$, $2$, $\infty$. A very good order of accuracy, close to $4$ is obtained for the norm $1$, $2$ and $\infty$.}
\label{tab:rate1}
\end{table} 
...
Two time step (associated with the number of time iteration) were used to obtain results at $T=12$ days. In both cases, the scheme
was found stable and the error grows smoothly.
For 2880 iterations, the error is dominated by the space approximation.
...
For 288 iterations, space and time errors are observed simultaneously.
The error level are slightly better for the bigger time step (288 iterations). This is a well known effect of the time stepping.
...
205 iteration; $(\lambda_C, \theta_C) = (\pi/4,\pi/4)$.}
\label{table:2.4}
\end{figure}
...
%\label{vitesse_theta_bump}
\end{equation}
where $\omega_s = 2 \pi / (12 days) $ and $( \lambda_C, \theta_C)$
...
The number of iterations is 457 (when $N=40$) and 914 (for $N=80$). The robustness of the scheme
...
error levels are compared to the DG scheme \cite{Nair-Jablonowski}. We compare the results at time $T=12$ days, with $\alpha=\pi/4$, $N=80$ and after $750$ iterations (at $T=12$ days) with the FV scheme \cite{Katta-Nair-Kumar}, this methods are at least fourth order and named WENO5 and KL4. Without filter, the WENO5 scheme error are $e_1=0.0021$, $e_2=0.0042$ and $e_{\infty} = 0.0191$, the KL scheme error are $e_1=0.0021$, $e_2=0.0043$ and $e_{\infty}=0.0194$ (with the filter, values are very close). With the present scheme, in the same context, the error are $e_1=1.67(-4)$, $e_2=7.23(-4)$ and $e_{\infty}=5.75(-3)$. This errors are smaller than the WENO5 and KL4.
...
% \includegraphics[scale=0.4]{ref_7367657290_normerreur_test_2.png}
 ...
 Error history of the relative error with $24$ days. Left panel: Grid resolution $N=40$ and $457$ time iterations. 
...
\caption{Convergence analysis for the Nair and Jablonowski test case \cite{Nair-Jablonowski}, $2 \pi u_0 \Delta t/N = 0.7$, $\alpha = \pi /4$.}
\label{table:2}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
