...
\end{center}
\caption{Convergence rate of the approximate relative vorticity 
of (\eqref{eq:test1}) for $e_p$ with $p=1,2$ and $\infty$.}
\label{fig:rate1}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This structure consists of two vortices, with symmetry with respect
...
\end{equation}
Let $T>0$, $\rho_0>0 $ and $ v_0 = 2 \pi R / T$
be parameters.
...
of two panels. This permits to assess 
the accuracy of the gradient calculation in Algorith \ref{algo:41}.
The resolution parameter is $N=35$, which corresponds to
a coarse grid, with equatorial resolution 
of $\Delta \lambda = 2.6 \deg$. 
Two $\CFL$ values were used. In both cases, the scheme
...
in the norms $p=1$, $p=2$ and $p=\infty$ with $\CFL=0.7$. The error convergence
...
$\CFL = 0.7$; $(\lambda_p, \theta_p) = (\pi/4,\pi/4)$.}
\label{table:2.4}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The CFL is $\CFL=0.9$. The robustness of the scheme
is clearly observed. Even on the coarse grid
$N=40$, the error level is approximately of $15\%$ at final time. 
On a fine grid of $N=80$, the $\max$ error is below $10\%$ at final time.
...
% \includegraphics[scale=0.4]{ref_7367657290_normerreur_test_2.png}
 \includegraphics[scale=0.4]{ref_7370917533_normerreur_test_2.png}
 \includegraphics[scale=0.4]{ref_7370917487_normerreur_test_2.png}
 \caption{Vortex test case of Nair and Jablonowski. 
 The axis $PP^\prime$ is such that 
 $(\lambda_P,  \theta_P) = (\pi, \pi / 4)$.
 Error history of the relative error with $24$ days. The $\CFL$ is $0.9$.
 Left panel: Grid resolution $N=40$ and $355$ time iterations. 
The relative error levels are after $24$ days: 
$L^1$ norm: $1.31 \%$,
$L^2$ norm: $3.61 \%$,
$L^\infty$ norm: $15.60 \%$,
  Right panel: Grid resolution $N=80$ and $711$ time iterations.
The relative error levels are after $24$ days: 
$L^1$ norm: $0.39 \%$,
$L^2$ norm: $1.48 \%$,
$L^\infty$ norm: $9.08 \%$,
...
In the computations we usee the constants
$a = 6.37122 \times 10^6\si{m} \text{(earth radius)}$,
$\Omega = 7.292 \times 10^{-5} \si{s^{-1}} \text{ earth angular velocity}$,
and $g = 9.80616 \si{m . s^{-2}} \text{ gravity constant}$.
