...
In the real of methods 
for fluid flows, it is clearly related to 
compact schemes for aeroacoustics \cite{Lele, Visbal-Gaitonde, 
Tam-Webb, Bogey-Bailly} and turbulence, \cite{Kim-Moin}.
In this sense, it belongs 
to a classical approach to numerical approximation, 
which can
...
This problem gives beyond the classical solid body rotation test case.
...
We use compact differentiation in its simplest form,
namely the standard fourth order compact scheme,
we briefley recall here.
...
and from \cite{Galewski-Scott-Polvani}.
...
In Table \ref{table:4}, a numerical grid convergence analysis is reported 
using three grids. 
The decreasing accuracy from $4$ to $3$ is attributed to
the loss of computer accuracy near $0$.
\begin{table}[ht!]
...
\hline 
\end{tabular}
\caption{Hermitian scheme applied to an exponential decaying solution of the LSWE. The final time is 1.5 hour.}
\label{table:4}
\end{table}
...
To the given 
parameter function $\theta \mapsto \varphi(\theta)=\exp \left( \dfrac{1}{(\theta-\theta_0)(\theta-\theta_1)} \right)$, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The equations (SW) \eqref{eq:swe} are solved using the scheme
...
test case of \cite{Galewski-Scott-Polvani}.
...
Table \ref{tab:W2 error order} clearly shows fourth order accuracy
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{figure}[ht!]
...
\includegraphics[scale=0.3]{ref_7367706276_snapshot_solution.png}
\includegraphics[scale=0.3]{ref_7367706276_snapshot_solution.png}
\caption{Numerical results of steady state geostrophic flow in direction of $\alpha=\pi/4$ on  $32 \times 32 \times 6$ grid after $5$ model days of simulation. Contour lines are plotted between 1150 m to 2950m with step of 200m (left). Contour lines of the relative spatial error (right) }
\label{fig:W2 alpha=pi/4}
\end{figure}

\begin{figure}[ht!]
\includegraphics[scale=0.4]{ref_7367706276_erreur.png}
...
\begin{remark}
We refer to \cite{Portelenelle-Croisille}
...
Numerical results of the height $h$ at times $5$, $10$ and $15$ days are given in figure \ref{fig:W5 snapshot} with a grid $6 \times 32 \times 32$. Numerical conservations are represented on Fig. \ref{fig:W5 conservation}.
...
\begin{figure}[ht!]
\includegraphics[scale=0.4]{ref_7367706559_conservationA.png}
\includegraphics[scale=0.4]{ref_7367706559_conservationB.png}
\caption{Conservation of isolated mountain test case with grid  $32 \times 32 \times 6$. \textbf{TIME STEP?????}}
...
On the two last figures, the error levels on the divergence and 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[ht!]
\includegraphics[scale=0.4]{ref_7367723128_conservationB.png}
\includegraphics[scale=0.4]{ref_7367723128_mass.png}\\
\includegraphics[scale=0.4]{ref_7367723128_energy.png}
\includegraphics[scale=0.4]{ref_7367723128_enstrophy.png}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The barotropic instability test, introduced in \cite{Galewski-Scott-Polvani}, is a perturbation of a steady state 
...
\dfrac{u_{max}}{e_n} \exp \left[ \dfrac{1}{(\tau - \theta_0)(\tau - \theta_1)} \right] 
...
at day $6$ for the Cubed Sphere parameters $N=32$, $N=64$ and $N=128$. 
...
In all the tim-dependent test cases, a minimal numerical diffusion is added
...
as part of the spatial operator in space in the form 
