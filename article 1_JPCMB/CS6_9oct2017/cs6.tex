...
of the equations.
\textbf{In the case of SWE, the vector height/momentum 
$q=[h,h\bv]$ is the vector of conservative variables. 
Such methods by construction
guarantee the conservation
of the components of the conservative vector $q$ over the full sphere.(C'EST VRAI CA?)} 
...
%% **********************************************************************************************************************************************
\subsection{Periodic compact differencing along great circles}
\label{sec:2.1}
...
%% **********************************************************************************************************************************************
\subsection{Time discretization and central filtering}
\label{sec:2.3}

Using the method of lines, the space discretization is made with the discrete operators \eqref{eq:grad} and \eqref{eq:div_rot}. Then, equations take the form 
\begin{equation}
\dfrac{d q}{dt} = J_{\Delta}(q)
\end{equation}
where $q = h$, restrict to the grid, for the advection equation \eqref{eq:adv}, $q=[\eta, \mathbf{v}]^T$ for the linearized shallow water equation \eqref{eq:lswe} and $q=[h, \mathbf{v}]^T$ for shallow water equation \eqref{eq:swe}. The time discretization is based on the classical Runge-Kutta order 4 scheme adding a filtering processus on times by iteration. The algorithm is given by the algorithm \ref{alg:RK4}.

\begin{center}
\begin{minipage}[H]{12cm}
  \begin{algorithm}[H]
    \caption{: Explicit Runge-Kutta Method's order 4 with filter}\label{alg:RK4}
    \begin{algorithmic}[1]
        \State $q^0 = q^{(0)}$ given
            \For{$n=0,1, \ldots$}
...
\end{minipage}
\end{center}
In the 7-th line, $\mathcal{F}$ denotes an hight frequency filtering. This high frequency filtering is intended to eliminate the +1/-1 mode attached to the grid and improves the stability properties \textbf{(See Annexes - TBA)}.
...
\end{equation}
This means that we apply two explicit filtering operators \textbf{ANNEXES} at each time step to all the components of the
vector $q$. The first one is along the direction $\xi$ and the second one is along the direction $\eta$. This filtering is
operated along great circles. This has been found as essential to preserve the symmetry of the computations
for large physical times.
...
that $\bJ$ is expressed in terms of the matrix $\bP \in \Bbb R^{N \times N}$ \textbf{A VERIFIER $P$ ou $P^T$}
\begin{equation}
...
With this condition, the filter remove the hight frequencies modes.
...
\beq
0 = \sum_{j=0}^J \dfrac{a_j}{2} (ij)^k\left( 1 + (-1)^k \right) \text{ for } k=1...K_{acc}.
\label{eq:accuracy_condition_filter}
\eeq

where $K_{acc} \in \mathbb{N}^{\star}$. When $K_{acc}$ is great the accuracy of the filter is great. Notice that for $k$ odd, the condition \eqref{eq:accuracy_condition_filter} become automatically true and \eqref{eq:accuracy_condition_filter} become :
\beq
...
A a = e_1
\end{equation}
with $e_1 = [1,0,\cdots,0]^T$. Expanding the second line
\begin{equation}
\det ( A ) = \begin{vmatrix} 
2  &  0  &  2  &  0  & 2  & \cdots\\
...
\end{proof}
We give some particular filters in the table \ref{tab:filter}.
...
\end{tabular}
\end{center}
\caption{Examples of filters in the form \eqref{eq:75.10.3} and their orders.}
\label{tab:filter}
\end{table}
...
\right\rbrace.
\eeq
These results are given in the tabular \ref{tab:CFL_filter}.
\begin{table}
\begin{tabular}{|c|c|}
\hline
...
\hline
\end{tabular}
\caption{CFL number for the scheme \eqref{eq:SF.55.19} in function of the explicit filter order. $"\infty"$ denotes the no filter scheme. The CFL is obtained numerically.}
