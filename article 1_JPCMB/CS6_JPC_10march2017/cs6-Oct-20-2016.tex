...
%approximation of the derivative along great circles of the Cubed-Sphere grid.
%This permits to define a discrete spherical gradient
%operator having high order accuracy.
...
%filtering. In this paper, the interest of this scheme is demonstrated  
%on two challenging vortex propagation test cases 
%of computational climatology \cite{Nair-Machenhauer, Nair-Jablonowski}.
%The numerical results show the interest of this approach for fluid flow
%problems relevant in computational climatology.
...
advection 
\beq
\partial_t h(t,\bx)+ \bc(t,\bx) . \nabla_T h(t,\bx)=0
\eeq
The velocity $(t,\bx) \in (0, +\infty) \times \Bbb S^^2$ is prescribed
so that the scalar value $h(t,\bx)$ exhibits a moving rollup vortex.
...
%As suggested in many works over the past 2o years, 
...
%% Despite the sucess of the latter schemes,
Note finally that Finite Difference methods were recently used
in numerical climatology in \cite{Ghader-Nordstrom}.
%\cite{Sjoegreen, Mattsson-Svard-Carpenter-Nordstrom, Mattsson-Svard-Shoeybi,
%BenArtzi-Croisille-Fishelov-4}.  Note also 
%that schemes based on special functions collocation
%using e.g. spectral functions, wavelets or Radial Basis functions
%insist
%on the relation with Finite-Difference formulas design.
%\cite{Fornberg-Lehto, Flyer-Fornberg, Fornberg-Merrill}.
...
on the role of the filtering. Dissipatyion and dispersion analysis 
...
The velocity $(\bx,t) \mapsto \bc(\bx,t)$ is prescribed as follows.
...
Let us define the velocity $\bx \in \Bbb S_R^2 \mapsto V(\bx)$ 
by
\begin{equation}
\left\{
\begin{array}{l}
\rho(\bx)=\rho_0 \cos(\theta^\prime)\\
V(\bx) = v_0 \dfrac{3 \sqrt{3} }{2} \sech^2 ( \rho ) \tanh ( \rho )
\end{array}
\right.
\end{equation} 
...
\end{equation}
Define now the tangential velocity on the sphere $\Bbb S^2_R$ 
appearing in (\ref{eq:978.23.1}) by
by:
\beq
\bc(\bx,t)=c_{\lambda^\prime} \be_{\lambda^\prime}+
c_{\theta^\prime} \be_{\theta^\prime}
\eeq
with
\beq
\left\{
...
\right.
\eeq
Switching back to the $(\be_\lambda,\be_\theta)$ basis, one obtains
\beq
\bc(\bx,t)=\bc(\bx)=c_{\lambda} \be_{\lambda}+
c_{\theta} \be_{\theta}
\eeq
with
\begin{equation}
...
\right.
\end{equation}
Now, the solution $(\bx, t) \in \Bbb S_R^2 \times \Bbb R_+ \mapsto
\phi(\bx,t)$ is given in coordinates $(\lambda^\prime,\theta^\prime,t)$ by
\begin{equation}
...
of the cornrs of the Cubed Sphere.
...
The advection velocity $\bc(\bx,t)$ in (\ref{eq:978.23.1}) is
obtained as the sum
\beq
\bc=\bc_s +\bc_r
\eeq
where $\bc_s$ is a solid rotation velocity and 
$\bc_r$ is a "static" velocity centered at the center 
of the vortex. The velocity $\bc_r$ is actually time dependant 
since in \ref{eq:78.10}
...
\beq
\tilde\bv(t,\bx)=\bu_0 \varphi(\theta) e^{-\sigma t}\be_\lambda(\bx)\\
\tilde\eta(t,\bx)= \eta_0 \varphi(\theta) \sin(\lambda)e^{-\sigma t} 
\eeq
...
(LSWE) \left\{
\begin{array}{l}
\partial_t \bv(t,\bx)+ \bg \nabla_T \eta(t,\bx) + f(\bx) \bk(\bx) \times
\bv(t,\bx)=S_{\eta}(t,bx)\\
\partial_t \eta(t,\bx)+ H \nabla_T . \bv(t,\bx)=S_{\bv}(t,\bx)
\end{array}
\right.
\eeq
In (\ref{eq:76.11}), $\bg$ is the gravity vector and $\bk(\bx)$ is the exterior
normal vector.
The source terms $S_{\eta}$ and $S_{\bv}$ are defined 
such as the functions $(\tilde \bv(t,\bx), \tilde \eta(t,\bx))$ be solution
of (\ref{eq:76.11}).
Remarks:\\
...
\hline 
$\vert \bv^1_{ex}(T)-\bv^1_{cal}(T)\vert_{h,I}$ & 2.52(-5)  &  4.15 & 4.69(-6) &  2.98 & 1.40(-6)   \\
\hline 
...
we define the 
spherical velocity $\bv(\bx)$ on the sphere by:
\beq
\bv(\bx)=u_0 \varphi(\theta) \be_\lambda(\bx)
\eeq
The momentum equation () is equivalent to the 
following relation:
%% The matching atmosphere thickness is $\eta(\bx)$:
\beq
\eta(\bx)=\eta_{eq}-\frac{a}{g}\int_0^\theta f(s) \varphi(s) ds
\eeq
...
\hline
$\vert \bv^1_{ex}(T)-\bv^1_{cal}(T)\vert_{h,I}$ & 2.73(-5)  &  4.09 & 5.18(-6) &  4.24 & 1.53(-6) \\
\hline 
