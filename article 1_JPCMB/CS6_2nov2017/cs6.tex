...
of using great circles on the Cubed Sphere. 
Two typical great circles ar represented
on FIG.1
...
\begin{tikzpicture}[scale=2.5]
	...
	\draw [samples=100,domain=0:1,color=blue] plot({\x},{1.5-0.35*cos(180*\x)});
	\draw [samples=100,domain=1:2,color=blue] plot({\x},{1.85});
	\draw [samples=100,domain=1:2,color=blue] plot({\x+1},{1.5-0.35*cos(180*\x)});
	\draw [samples=100,domain=3:4,color=blue] plot({\x},{1.15});
	\draw [>=stealth, <-] (0.1,1.167) -- (0.3,0.5) ;
	\draw  (0.3,0.5) node[right] {iso-$\xi$} ;
	
	\draw [samples=100,domain=0:1,color=red] plot({1.5-0.3*cos(180*\x)},{\x});
	\draw [samples=100,domain=1:2,color=red] plot({1.8},{\x});
	\draw [samples=100,domain=1:2,color=red] plot({1.5-0.3*cos(180*\x)},{\x+1});
	\draw [samples=100,domain=1:2,color=red] plot({3.2},{\x});
	\draw [>=stealth, <-] (1.5,0.5) -- (2.2,0.4) ;
	\draw  (2.2,0.4) node[right] {iso-$\eta$} ;
	
	\draw  (0,1.15) node[color=blue] {$\bullet$} ;
	\draw (0,1.15) node {$\circ$}
	\foreach \k in {0,...,5}
		{\draw  (1+0.2*\k,1.85) node[color=blue] {$\bullet$} ;
	   	\draw (1+0.2*\k,1.85) node {$\circ$} ;
	   	\draw  (3+0.2*\k,1.15) node[color=blue] {$\bullet$} ;
	   	\draw (3+0.2*\k,1.15) node {$\circ$} ;
	   	}
	   	
	\foreach \x in {0,...,5}
		{\draw  ({0.2*\x},{1.5-0.35*cos(180*0.2*\x)}) node[color=blue] {$\bullet$} ;
	   	\draw ({0.2*\x},{1.5-0.35*cos(180*0.2*\x)}) node {$\circ$} ;
	   	\draw  (3+0.2*\k,1.15) node[color=blue] {$\bullet$} ;
	   	\draw (3+0.2*\k,1.15) node {$\circ$} ;
	   	}
...
Consider a regular function $v : \mathbf{x} \in \mathbb{S}^2_a \mapsto v(x) \in \mathbb{R}$ on the sphere.
The gradient of $v(x)$ is
\begin{equation}
\nabla_T v = \dfrac{\partial v}{\partial \xi} \mathbf{g}^{\xi} + \dfrac{\partial v}{\partial \eta} \mathbf{g}^{\eta}
\label{eq:gradient}
\end{equation}
Consider the restriction $v^k_{i,j} = v(\mathbf{s}_{i,j}^k)$ to the Cubed-sphere. The approximate gradient is
\begin{equation}
\nabla_{T,h} v_{i,j}^k =\delta_{\xi}^H v_{i,j}^k \mathbf{g}^{\xi} + \delta_{\eta}^H v_{i,j}^k \mathbf{g}^{\eta}
\label{eq:gradient_app}
\end{equation}
In \eqref{eq:gradient_app}, the partial derivative $\frac{\partial}{\partial \xi}$ is approximated by the compact finite difference $\delta_{\xi}^H$ along a great circle. See appendix.

%%%%%%%%%%%%%%%%%%%%%% ??????????????????????????

Condiser for example the approximation of the derivative $\partial_{xi}v_{i,j}^k$. We use the blue circle on figure FIG.1 to illustrate the algorithm. This circle correspond to the iso-$\eta$ line $\eta=\eta_0$.

\begin{enumerate}
\item Define on the blue circle the grid $p = 0, ..., 4N$ as follow :
\begin{enumerate}
\item $m_p = s^{(I)}_{p-N/2,j_0}$, $p=0, ..., N-1$. These $N$ points are represented by circles on FIG.1. They belong to the Cubed-Sphere,
\item $m_p$, $p=N...2N-1$. These points are represented by blue squares on Fig.1. They do not belong the Cubed-Sphere.
\item $m_p = s^{(III)}_{p-N/2,j_0}$, $p=2N, ..., 3N-1$. These $N$ points are represented by circles on FIG.1. They belong to the Cubed-Sphere,
\item $m_p$, $p=3N...4N-1$. These points are represented by red squares on Fig.1. They do not belong the Cubed-Sphere.
\end{enumerate}
...
\item The hermitian discrete derivative REF is applied to the data assembled along great the blue circle. This step provides $4N$ values called $v_{\xi, p}$, with $p=0, ..., 4N$. Periodicity ensures that $v_{\xi, 4N} = v_{\xi,0}$.
...
ICI DONNER LES CERCLES.

As a result, approximate values for $v_{\xi, i, j}$ and $v_{\eta, i, j}$ are obtained on the six panels. Using REF, this allow to obtain an approximation of the gradient at each point of the Cubed-sphere.
...
\label{eq:div_rot}
\eeq
Due to (\ref{eq:herm_accurate}), we expect these discrete operators
...
For example, for the shallow water equation \eqref{eq:swe}, $q = [h_{i,j}^k, \mathbf{v}_{i,j}^k]^T$ with $-N/2 \leq i,j \leq N/2$ and $k = (I), ..., (VI)$. Each operator is approximated using REF. Then
\begin{equation}
...
\end{equation}
The time discretization is based on the classical Runge-Kutta order 4 scheme adding a filtering processus on times by iteration. The algorithm is given by the algorithm \ref{alg:RK4F}.
...
vector $q$. The first one is along the direction $\xi$ and the second one is along the direction $\eta$ along complete great circles. As for the derivatives, the data are completed by an interpolation procedure.
...
given in algorithm \ref{alg:RK4}.
...
is used with the filtered RK4 scheme (algorithm \ref{alg:RK4}).
...
(\ref{eq:grad}-\ref{eq:div_rot}) with the algorithm \ref{alg:RK4}.
